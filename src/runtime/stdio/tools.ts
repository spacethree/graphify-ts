...
import { compactRetrieveResultForStdio, retrieveContext } from '../retrieve.js'
...
export function handleToolCall(id: string | number | null, graphPath: string, params: unknown, helpers: ToolHelpers): StdioResponse | Promise<StdioResponse> {
  ...
  switch (toolName) {
    ...
    case 'retrieve': {
      ...
      return helpers.ok(id, helpers.textToolResult(JSON.stringify(compactRetrieveResultForStdio(result))))
    }
