...
> **Status update (2026-04-13):** The first implementation slices from this plan are now complete and verified: a fixture-backed schema-v2 contract test suite, additive `schema_version` / `layer` / `provenance` validation, a builtin capability-registry foundation with normalized extension lookup plus source-classification-aware disambiguation and duplicate-claim protection, immutable schema normalization helpers that upgrade legacy payloads in the graph-build path, registry-driven single-file extraction dispatch in `src/pipeline/extract/dispatch.ts`, registry-driven multi-file extraction aggregation in `src/pipeline/extract/combine.ts`, registry-driven Python cross-file linking in `src/pipeline/extract/cross-file.ts`, registry-driven ingest dispatch in `src/infrastructure/ingest/dispatch.ts`, explicit top-level `schema_version` preservation through `build()`, `graph.json` export, graph reload, and incremental `generate --update` rebuilds, helper-created raw extraction output that now emits explicit base-layer/provenance metadata before normalization, extractor-cache invalidation for stale pre-metadata raw payloads, explicit raw ingest-provenance emission for registry-driven text captures while keeping captured files backward-compatible on disk, normalization-time ingest-provenance fallback/dedupe for backward compatibility, hidden binary PDF/image ingest sidecars plus sidecar-aware extractor cache hashing so extracted file nodes can lift flat ingest metadata without changing the returned asset path, explicit raw ingest-provenance fan-out across sidecar-backed PDF/image/DOCX/XLSX extraction (including corrupted-DOCX file-only fallback), and broader markdown/PDF/DOCX citation-linkage regressions covering LaTeX multi-key splitting, arXiv format variants, bibliography-entry citation links, bibliography-section detection, and inline numeric forward-reference resolution. The previous Phase 1 metadata-emission gap for the current builtin non-code families is now closed; future extractor or ingest additions should follow the same explicit raw-emission discipline.
...
> **Task 3 status update (2026-04-13):** The current verified slice now preserves top-level `schema_version` across `build()`, `graph.json` export, `loadGraph()`, and incremental `generate --update`, with regression coverage in `tests/unit/build.test.ts`, `tests/unit/schema-v2.test.ts`, `tests/unit/serve.test.ts`, and `tests/unit/generate.test.ts`. The top-level schema-preservation risk is closed for the current builtin families; remaining work here is mostly about keeping future extractor families on the same explicit `layer` / `provenance` discipline rather than preventing silent downgrade.
...
> **Task 6 status update (2026-04-13):** Step 1 is now shipped: `src/infrastructure/ingest/dispatch.ts` resolves URL kinds through the builtin capability registry, `src/infrastructure/ingest.ts` retains the existing fetch/download helpers behind registry-selected handlers, and focused regression coverage now includes builtin ingest-capability resolution, dispatch fallback behavior, flat tweet/X plus arXiv frontmatter compatibility, hidden binary PDF/image sidecars for direct downloads, sidecar-aware extractor cache invalidation, and URL-type-specific tweet/X, arXiv, PDF, and image provenance selection during normalization in `tests/unit/capability-registry.test.ts`, `tests/unit/ingest-dispatch.test.ts`, `tests/unit/ingest.test.ts`, `tests/unit/extract.test.ts`, and `tests/unit/normalize.test.ts`. Step 3 is now shipped more broadly: flat ingest frontmatter remains backward-compatible on disk for text captures, `src/pipeline/extract/non-code.ts` now emits explicit ingest provenance during raw extraction for registry-driven text captures, `src/core/schema/normalize.ts` remains the fallback/dedupe path for older payloads, direct binary downloads now persist hidden sidecars so extracted file nodes can lift the same flat metadata without changing the returned asset path, and sidecar-backed PDF/image/DOCX/XLSX extraction (including corrupted-DOCX file-only fallback) now fans out explicit raw ingest provenance before normalization. The previous remaining work here is now closed for the current builtin families; future extractor or ingest additions should preserve the same explicit raw-emission discipline.
