...
**Progress update (2026-04-15):** `src/pipeline/detect.ts`, `src/pipeline/extract/non-code.ts`, `src/infrastructure/generate.ts`, `src/infrastructure/watch.ts`, `src/pipeline/analyze.ts`, `src/infrastructure/ingest.ts`, `src/infrastructure/ingest/url-type.ts`, `src/infrastructure/capabilities.ts`, `src/shared/security.ts`, and `src/shared/binary-ingest-sidecar.ts`, plus `src/core/provenance/ingest.ts`, now treat common local audio/video extensions as first-class extractable file types, let direct binary audio/video URLs download into that same sidecar-backed path, and let webpage-shaped or redirected asset URLs fall back into the binary ingest path when the final URL or response content type confirms PDF/image/audio/video content. Local or directly ingested media files plus saved PDF captures now land as base-layer `audio` / `video` / `paper` file nodes with hidden sidecar `source_url` / `captured_at` / `contributor` metadata plus explicit binary ingest-kind hints for extensionless assets, lift deterministic extension-derived `content_type` plus saved-asset `file_bytes` metadata, add lightweight `media_duration_seconds` metadata for WAV plus common MP4-family assets with recognizable top-level container headers, including bounded `mdhd` fallback when `mvhd` does not yield duration, add deterministic MP4-family video `video_width_px` / `video_height_px` plus embedded audio-track `audio_sample_rate_hz` / `audio_channel_count` metadata from bounded `moov` parsing, including `tkhd` width/height fallback when the visual sample entry path does not yield dimensions, add deterministic AVI `media_duration_seconds`, `video_width_px`, and `video_height_px` plus embedded audio-track `audio_sample_rate_hz` / `audio_channel_count` metadata from bounded RIFF parsing, add WAV `audio_sample_rate_hz` / `audio_channel_count` metadata when the declared WAV data chunk is actually present, add common MP3 ID3 `audio_title` / `audio_artist` / `audio_album` metadata, add deterministic AAC ADTS `media_duration_seconds`, `audio_sample_rate_hz`, and `audio_channel_count` metadata, add richer M4A MP4-container `audio_sample_rate_hz`, `audio_channel_count`, `audio_title`, `audio_artist`, and `audio_album` metadata from deterministic sample-entry and `ilst` parsing, add deterministic Matroska/WebM `media_duration_seconds`, `video_width_px`, and `video_height_px` plus embedded audio-track `audio_sample_rate_hz` / `audio_channel_count` metadata from a bounded 512 KiB EBML head scan plus a capped set of bounded `SeekHead` rereads and a bounded later top-level segment scan that can still recover missing, truncated, or unhinted `Info` / `Tracks` after large prefixed segment content while still combining split or corrective hints across multiple `SeekHead` elements, and can also reuse already-buffered parseable metadata from the first direct `Info` or `Tracks` prefix when its header is visible in the head window, including `Info` prefixes whose remaining payload is only trailing padding beyond the bounded windows and whose parseable direct prefix can now keep duration recoverable, while later direct or later top-level exact-boundary trailing-child `Info` targets keep corrected duration recoverable in the same bounded path and also clear stale duration when `Duration` is omitted or malformed before the trailing child, while later-top-level overrun or truncated trailing child headers after omitted or malformed `Duration` remain unreadable and preserve earlier valid duration, and `Tracks` prefixes whose remaining payload is only trailing padding beyond the bounded windows and whose parseable direct prefix can now reuse buffered metadata or replace stale earlier track metadata, or self-reread that same target when the buffered prefix is insufficient and no useful `SeekHead` exists, so later authoritative direct targets still replace stale earlier duration or track metadata or clear stale duration, audio, or video-dimension metadata when the later authoritative reread omits those fields, including direct `Tracks` prefixes whose exact-boundary trailing child can still reuse buffered parseable metadata, replace stale earlier track metadata, or clear stale audio-track or video-dimension fields, while later top-level exact-boundary trailing-child `Tracks` targets now also replace stale earlier track metadata or clear stale audio-track or video-dimension fields in the same bounded path, while later top-level overrun or truncated trailing-child `Tracks` targets remain unreadable and preserve earlier valid track metadata, and later direct `Info` prefixes whose buffered children already prove `Duration` is invalid or omitted when a trailing metadata child or padding reaches the exact `Info` boundary, while malformed overrun or truncated trailing child headers after omitted or invalid `Duration` remain unreadable and preserve earlier valid duration, while later unreadable `Info` targets now also preserve earlier valid duration for stale-direct no-`SeekHead` cases and later unreadable `Tracks` targets preserve earlier valid dimension/audio-track metadata, including direct `Tracks` prefixes whose trailing child header is malformed or truncated, plus the parallel stale-direct no-`SeekHead` direct-`Tracks` reread path now locked in as regression coverage only without another cache bump, add deterministic FLAC STREAMINFO/Vorbis-comment and Ogg Vorbis/Opus identification/comment/granule metadata for `media_duration_seconds`, `audio_sample_rate_hz`, `audio_channel_count`, `audio_title`, `audio_artist`, and `audio_album`, plus Ogg-family BOS-stream selection within a bounded 512 KiB head window that skips leading non-audio streams before supported Vorbis or Opus audio even when a larger prefixed logical stream pushes the supported BOS beyond the old window, carry final-asset `source_url` metadata when binary redirects resolve to a more canonical asset URL, keep fallback HTML canonical URLs resolved against the final response URL, participate in `generate --update`, watch-mode rebuilds, and report categorization, invalidate stale pre-Matroska-direct-tracks-prefix-tail-truncated media payloads via extractor cache version 45 from the prior direct-`Tracks` trailing-child truncated-header hardening, and keep the earlier no-`SeekHead` direct-truncated-target, stale-direct-correction, truncated-direct authoritative-clearing, unreadable-direct-`Tracks` stale-direct, direct-`Tracks` trailing-padding direct-prefix, authoritative-clear direct-`Tracks` trailing-padding, stale-direct direct-`Info` trailing-padding, and direct-`Info` trailing-child direct-target duration recovery, stale-direct correction, plus invalid-duration and omitted-duration truncated-header proofs, plus later-top-level trailing-child stale-direct correction, omitted-duration clear, invalid-duration clear, invalid-duration overrun-preservation, invalid-duration truncated-header preservation, omitted-duration overrun preservation, omitted-duration truncated-header preservation, and stale-direct `Tracks` correction coverage and authoritative audio-track clearing coverage and authoritative video-dimension clearing coverage and stale-track overrun-preservation coverage and stale-track truncated-header-preservation coverage and later direct-`Tracks` trailing-child overrun proof coverage and direct-`Tracks` trailing-child direct-prefix reuse plus stale-direct correction and authoritative audio-clear and video-dimension-clear proof coverage, as regression coverage only without another cache bump, and stay intentionally scoped to deterministic file-node support without transcription or segment nodes yet. The later-top-level trailing-child `Tracks` proof matrix is now complete; the next broader frontier is deepening Rust AST-backed coverage with trait/conformance edges and aliased `use` forms under Workstream 7 before moving on to C#.
...
**Progress update (2026-04-17):** Rust now joins the portable tree-sitter family for a first bounded slice covering namespaced module/type ownership, struct/impl/top-level function structure, module-local trait signature extraction, `impl Trait for Type` conformance edges including single-segment traits brought into scope via `use`, simple and grouped-leaf `use` / `pub use` import lifting plus single-target aliased `use_as_clause` bindings, cross-file `use crate::...` trait/type target reuse where the current stable-id scheme lines up with another Rust file, nested module-scoped `imports_from` edges that stay on the importing Rust module instead of collapsing onto the file, order-independent qualified `impl` owner binding, and common `self.` / `Type::method()` call coverage including nested `super::` relative paths and anchor-only `super::name()` free-function calls in the existing fixture-backed cases without qualified-path fallback onto unrelated free functions, while inherent `impl Type { ... }` blocks do not emit bogus self-conformance edges; extractor cache version 59 invalidates stale pre-nested-import-scope Rust payloads. No next post-Rust language slice is currently seeded; C# is deferred for now.
