...
- first local audio/video file-node support across common media extensions, plus direct PDF/image/audio/video or webpage-shaped binary asset URLs that resolve into the same hidden-sidecar path, with deterministic extension-derived `content_type`, saved-asset `file_bytes`, lightweight duration metadata for WAV plus common MP4-family assets with recognizable top-level container headers, deterministic MP4-family video `video_width_px` / `video_height_px` metadata from visual sample entries, deterministic Matroska/WebM `media_duration_seconds` / `video_width_px` / `video_height_px` metadata from bounded EBML parsing, common MP3 ID3 `audio_title` / `audio_artist` / `audio_album` metadata lift, deterministic AAC ADTS `media_duration_seconds` / `audio_sample_rate_hz` / `audio_channel_count` metadata, richer M4A MP4-container `audio_sample_rate_hz` / `audio_channel_count` / `audio_title` / `audio_artist` / `audio_album` metadata lift, deterministic FLAC/Ogg Vorbis/Opus `media_duration_seconds` / `audio_sample_rate_hz` / `audio_channel_count` / `audio_title` / `audio_artist` / `audio_album` metadata lift, graph/report categorization, and watch/update coverage but no transcription yet
