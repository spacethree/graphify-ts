...
- extraction MVP for:
  ...
  - Go and Java via portable `web-tree-sitter` + `@vscode/tree-sitter-wasm` parsing for owner-aware type/method extraction, imports, and common call-graph edges
  - remaining brace-style languages (currently including Kotlin, Scala, C#, Rust, Swift, PHP, Zig, and C/C++-style signatures) via a portable regex-based structural extractor with common inheritance/conformance support
...
- expand tree-sitter coverage beyond the current Go/Java AST-backed slice, especially for stronger Python/reference-parser parity
- richer MCP/stdIO runtime parity beyond the current prompt/resource/tool surface, snake_case aliases, and `god_nodes` support
- broader CLI/runtime parity for deeper runtime features beyond the current transport-selection and installer/runtime surfaces
...
- Go and Java extraction now run through a portable WASM tree-sitter path instead of heuristics. The current AST-backed slice focuses on common type, interface, record, method, import, and call-graph patterns already covered by tests; deeper language-specific edge cases still need more parity work.
...
- A direct Node native `tree-sitter` install failed in this environment under Node 25, so the TS port currently uses portable WASM tree-sitter for Go/Java and keeps non-AST fallbacks elsewhere instead of relying on native parser addons.
...
- widen portable tree-sitter coverage beyond the current Go/Java slice, or add a stronger parser strategy for Python parity
